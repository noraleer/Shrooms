...
dfpred <- data.frame(
  mushroomtest,
  pred = predict(mushroom_tree1, mushroomtest, type = "response")
)

```

```{r}
pairs <- combn(names(mushrooms), 2, simplify = FALSE)

perfect_assoc <- Filter(Negate(is.null),
  lapply(pairs, function(p) {
    tab <- table(mushrooms[[p[1]]], mushrooms[[p[2]]])
    if(all(rowSums(tab > 0) == 1) && all(colSums(tab > 0) == 1)) return(p)
  })
)
