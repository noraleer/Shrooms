...
mushrooms <- mushrooms %>% dplyr::select(-c(veil_type, stalk_color_above_ring, stalk_color_below_ring)) %>%
  dplyr::mutate(class = case_when(class == "edible" ~ 0, class == "poisonous" ~ 1))

mushrooms <- mushrooms %>% dplyr::select(-odor)
mushrooms <- mushrooms %>% dplyr::select(-spore_print_color)
mushrooms <- mushrooms %>% dplyr::select(-gill_size)
mushrooms <- mushrooms %>% dplyr::select(-ring_type)
mushrooms <- mushrooms %>% dplyr::select(-cap_surface)
mushrooms <- mushrooms %>% dplyr::select(-gill_attachment)
mushrooms <- mushrooms %>% dplyr::select(-stalk_root)
mushrooms <- mushrooms %>% dplyr::select(-stalk_surface_above_ring)
mushrooms <- mushrooms %>% dplyr::select(-stalk_surface_below_ring)
mushrooms <- mushrooms %>% dplyr::select(-ring_number)

#write.csv(mushrooms, "mushrooms_clean1.csv")
...
mushroom_idx = sample(1:nrow(mushrooms),nrow(mushrooms)*.7)
mushroomtrain <- mushrooms[mushroom_idx,]
mushroomtest <- mushrooms[-mushroom_idx,]


mushroomlm <- glm(class ~ ., data = mushroomtrain, family = "binomial")

summary(mushroomlm)

dfpred <- data.frame(
  mushroomtest,
  pred = predict(mushroomlm, mushroomtest, type = "response")
)

auc(roc(dfpred$class, dfpred$pred))
...
mushroom_tree2 <- rpart(class ~ ., data = mushroomtrain, cp =  0.00254176)
#predict(titanic_tree1)

printcp(mushroom_tree2)

rpart.plot(mushroom_tree2)


mushroom_tree3 <- rpart(class ~ ., data = mushroomtrain, cp =  0.0078673)
#predict(titanic_tree1)

printcp(mushroom_tree3)

rpart.plot(mushroom_tree3)
...
mushroomtest$class <- as.factor(mushroomtest$class)

rfmushroom <- randomForest(class ~ ., data = mushroomtest)


rfmushroom
plot(rfmushroom)

plot(rfmushroom$err.rate[, "OOB"], 
     type = "l", 
...
rfmushroom2 <- randomForest(class ~ ., data = mushroomtrain, ntree = 5)
